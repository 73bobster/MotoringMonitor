...
function getCache() { return readCache(CACHE_KEY, { people: [], vehicles: [], readings: [], runningCosts: [], drivingRecords: [], namedDrivers: [] }); }
...
async function refreshFromCloud() {
  ...
  const [people, vehicles, readings, runningCosts, drivingRecords, namedDrivers] = await Promise.all([
...
function exportCSV() {
  const cache = getCache();
  const vehiclesCSV = toCSV(cache.vehicles, ['nickname','registration','make','model','ownership_type','mpg']);
  const readingsCSV = toCSV(cache.readings, ['vehicle_id','date','mileage']);
  const costsCSV = toCSV(cache.runningCosts, ['vehicle_id','category','date','cost','note']);
  const blob = new Blob(
    [`VEHICLES\r\n${vehiclesCSV}\r\n\r\nREADINGS\r\n${readingsCSV}\r\n\r\nRUNNING COSTS\r\n${costsCSV}\r\n`],
    { type: 'text/csv' }
  );
