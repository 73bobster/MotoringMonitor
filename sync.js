...
function getCache() { return readCache(CACHE_KEY, { people: [], vehicles: [], readings: [], runningCosts: [], drivingRecords: [], namedDrivers: [], insurancePolicies: [], documents: [] }); }
...
async function refreshFromCloud() {
  ...
  const [people, vehicles, readings, runningCosts, drivingRecords, namedDrivers, insurancePolicies, documents] = await Promise.all([
    ...
    supabaseClient.from('documents').select('*').eq('household_id', householdId),
  ]);
  const cache = {
    ...
    insurancePolicies: insurancePolicies.data || [], documents: documents.data || [],
  };
  setCache(cache);
...
function exportCSV() {
  ...
  const vColumns = ['registration','nickname','make','model','variant','fuel_type','engine_capacity','year_of_manufacture','first_registered_date','mpg',
  ...
  const peopleCSV = toCSV(cache.people, ['nickname','email','mobile','licence_start_date']);
  ...
  const policiesCSV = toCSV((cache.insurancePolicies||[]).map(p => ({ ...p, vehicle: vehicleRef(p.vehicle_id) })),
    ['vehicle','insurer','policy_number','annual_cost','start_date','end_date','website','claims_phone','general_phone','is_current']);

  const recordsCSV = toCSV(cache.drivingRecords.map(r => ({ ...r, person: personRef(r.person_id) })),
    ['person','kind','offence_code','points','fine','date','totting_up_clear_date','licence_clear_date','insurance_disclosure_date','description','cost','at_fault','claim_reference','comments']);
...
async function uploadDocument(vehicleId, category, file) {
  if (!navigator.onLine) throw new Error('Uploading a file needs an internet connection — can\'t be queued offline.');
  const path = `${householdId}/${vehicleId}/${Date.now()}-${file.name}`;
  const { error: uploadErr } = await supabaseClient.storage.from('documents').upload(path, file);
  if (uploadErr) throw uploadErr;
  const doc = {
    id: crypto.randomUUID(), household_id: householdId, vehicle_id: vehicleId,
    category, file_name: file.name, storage_path: path, file_size: file.size,
    uploaded_by: currentUser.id, created_at: new Date().toISOString(),
  };
  const { error: insertErr } = await supabaseClient.from('documents').insert(doc);
...
async function getDocumentSignedUrl(storagePath) {
  const { data, error } = await supabaseClient.storage.from('documents').createSignedUrl(storagePath, 300);
  if (error) throw error;
  return data.signedUrl;
}
async function deleteDocument(id) {
  if (myRole !== 'owner') throw new Error('Only the household owner can delete documents');
  const cache = getCache();
  const doc = (cache.documents || []).find(d => d.id === id);
  if (doc && navigator.onLine) {
    await supabaseClient.storage.from('documents').remove([doc.storage_path]);
  }
